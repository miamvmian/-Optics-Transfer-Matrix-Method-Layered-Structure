#!/usr/bin/env python
"""
Comprehensive energy conservation test for transfer matrix calculations.

Tests R + T = 1.0 for various scenarios:
...
for pol in ["s", "p"]:
    ...
    print(
        f"  {pol}-polarization: {status} | R+T = {R_plus_T} | Max deviation: {max_dev:.2e}"
    )
...
for n_layers in range(1, 11):
    ...
    for pol in ["s", "p"]:
        ...
        if not passed:
            status = "✗ FAIL"
            print(
                f"  {n_layers} layers, {pol}-pol: {status} | Max deviation: {max_dev:.2e} | R+T = {np.mean(R_plus_T):.10f}"
            )
...
for scenario, results in sorted(scenarios.items()):
    s_status = (
        f"✓ ({results['s'][1]:.2e})"
        if results["s"] and results["s"][0]
        else f"✗ ({results['s'][1]:.2e})" if results["s"] else "N/A"
    )
    p_status = (
        f"✓ ({results['p'][1]:.2e})"
        if results["p"] and results["p"][0]
        else f"✗ ({results['p'][1]:.2e})" if results["p"] else "N/A"
    )
...
if failed_tests == 0:
    ...
    print(
        f"  All {total_tests} test cases show R + T = 1.0 within tolerance ({tolerance})"
    )
else:
